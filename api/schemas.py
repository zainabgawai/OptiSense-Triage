"""
schemas.py — OptiSense Triage API request/response schemas
===========================================================
All Pydantic v2 models used by api/main.py.
 
Design notes:
  - TriageRequest  : what the nurse dashboard POSTs to /triage
  - TriageResponse : the full structured prediction returned by /triage
  - QueueEntry     : one patient in the live queue (GET /queue)
  - QueueResponse  : the full queue response
  - BedStatus      : one bed's current state (GET /beds)
  - BedsResponse   : the full beds response with per-category summary
"""
...
# ─── Comorbidities ──────────────────────────────────────────────────────
# Binary flags for pre-existing conditions known at triage time.
...
class TriageRequest(BaseModel):
    ...
    # Free-text chief complaint — used in two ways:
    #   1. Encoded via all-MiniLM-L6-v2 → PCA(8 dims) → complaint_emb_* model features
    #   2. Passed verbatim to Gemini 2.5 Flash for clinical reasoning
    # Falls back gracefully to complaint_cat keyword encoding if omitted or if
    # the sentence-transformer model is not available.
...
class TriageResponse(BaseModel):
    ...
    # These are the direct outputs of the XGBoost classifier before any
    # post-processing. Use critical_probability for downstream thresholding.
    ...
    # Triggered when the model is uncertain OR when the model and vital signs
    # give conflicting acuity signals. Treat this as a mandatory escalation trigger.
    ...
    # Generated by Gemini 2.5 Flash using vitals + signal engine output as context.
    ...
    # Rule-based findings computed independently from the model.
    # critical_flags  : the most dangerous findings as plain strings
    # context_flags   : comorbidity / demographic risk factors
    # clinical_signals: the full structured list (includes severity + clinical note)
