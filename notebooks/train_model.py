"""
train_model.py — OptiSense Triage binary classifier
====================================================
Trains a binary XGBoost model that decides whether a patient is
CRITICAL (ESI 1-2) or NON-CRITICAL (ESI 3-5).
 
WHY BINARY?
-----------
The MIMIC-III demo dataset has only ~70 admissions. After splitting,
ESI-1 has ~3 training examples and ESI-4 has ~2. A 5-class model cannot
learn from that — it collapses to always predicting the majority class (ESI-2).
Framing it as binary gives us 47 critical vs 23 non-critical, which is
feasible for honest training.
 
The granular ESI 1-5 score shown in the API is produced DOWNSTREAM by the
clinical override layer in api/main.py, which applies hard vital-sign rules.
This mirrors real clinical decision-support architecture: ML provides a prior,
rule-based safety checks provide the floor.
 
INPUTS  (run pipeline.py first)
  mimic-iii-clinical-db/mimic-iii-train.csv
  mimic-iii-clinical-db/mimic-iii-real.csv
 
OUTPUTS
  models/triage_model_binary.pkl       — calibrated binary XGBoost classifier
  models/feature_names_binary.pkl      — ordered feature list (used by main.py)
  models/real_median_binary.pkl        — feature medians for NaN imputation at inference
  models/train_median_binary.pkl       — training medians (diagnostic reference)
  models/evaluation_report_binary.txt  — F1 + safety recall metrics
  reports/confusion_matrix_binary.png  — confusion matrix plot
"""
...
# ─── LOAD ───────────────────────────────────────────────────────────────
df_real  = pd.read_csv("mimic-iii-clinical-db/mimic-iii-real.csv")         # held-out test set
df_train = pd.read_csv("mimic-iii-clinical-db/mimic-iii-train.csv")        # augmented training set
...
# ─── BINARY LABEL ───────────────────────────────────────────────────────
# 1 = critical   (ESI 1 or 2) — needs immediate or emergent care
# 0 = non_critical (ESI 3, 4, or 5) — can safely wait for resources
...
# to reduce overfitting on the dataset.
# sample_weight="balanced" gives higher weight to the minority critical class
# so the model doesn't bias toward the majority non-critical class.
...
plt.close()
print("\nConfusion matrix saved -> reports/confusion_matrix_binary.png")
